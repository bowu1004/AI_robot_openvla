...
if os_name == 'Windows':
    ...
    dllPath = os.path.join(CUR_PATH, "libRM_Base.so")  # 4.2.7
else:
...
class Move_Plan():
    ...
    def Movej_P_Cmd(self, pose, v, r=0, trajectory_connect=0, block=True):
        """
        该函数用于关节空间运动到目标位姿
...
-
    ...
    def Get_Gripper_State(self, retry=5):
        """
        Get_Gripper_State 查询夹爪状态
        :return:0-成功，失败返回:错误码, rm_define.h查询.
        """
        ...
        tag = self.pDll.Get_Gripper_State(self.nSocket, gripper_state)

        while tag and retry:
            # logger_.info(f'Get_Gripper_State:{tag},retry is :{retry}')
            tag = self.pDll.Get_Gripper_State(self.nSocket, gripper_state)
            retry -= 1

        # logger_.info(f'Get_Gripper_State:{tag}')

        return tag, gripper_state
